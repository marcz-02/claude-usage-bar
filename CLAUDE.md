...
`utilization` from tier 1/2 is Claude's own UI number — authoritative, no calibration needed. The menu's **Source** line shows which tier produced the current value (`live (API, Xs ago)`, `live (Cache, Xs ago)`, or `estimate (buddy.json)`).
...
Since only a percentage is available, the app makes the "fills up faster" feeling measurable. Every tick with a genuine live value appends `{ts, util}` to `~/.claude_token_ring_usage_log.jsonl` (`_append_usage_log`, pruned to `RATE_LOG_MAX = 17280` lines ≈ 6 months at 30 s intervals). `_usage_rate()` computes %/h over the last `RATE_WINDOW_SEC` (30 min), restarting the window after any drop > `RATE_RESET_DROP` (a 5h reset). `_update_icon` shows `Rate: +X%/h` and projects `Full in: ~Yh Zm` (with `(after reset)` appended when the projection lands past the 5h reset). Fallback estimates are never logged — only real live values.
...
Draws the 5h utilisation chart inside `Advanced ▸` (300×120 px, injected via `_install_chart_item`). X-axis: 0–5h of the current session. Y-axis: 0–100%. Blue line = actual samples from `usage_log.jsonl`; orange dashed = projection to 100% based on current rate; red horizontal line at 100%.
...
HTML/Canvas preview for tuning the ring visuals. Open via `python3 -m http.server 7823` and navigate to `http://localhost:7823`.

### `make_gif.py`

Generates the two animated GIFs used in the README (`screenshots/ring-dark.gif`, `screenshots/ring-light.gif`). Both are 88×88 px with transparent backgrounds (GIF palette index 0) and 70 frames. Requires the pre-rendered icon cache at `/tmp/claude_ring_icons/` (populated at app startup).

```bash
/Library/Developer/CommandLineTools/usr/bin/python3 make_gif.py
```

- `ring-dark.gif`  — white ring on transparent (shown in GitHub dark mode)
- `ring-light.gif` — dark `(28,28,30)` ring on transparent (shown in GitHub light mode)
...
| Item | Action |
|---|---|
| `5h usage` / `Reset` / `Rate` / `Full in` | Live 5h-window readouts (no callback) |
| `7-day usage` / `Reset 7d` / `Source` | Live 7-day readouts + data source indicator |
...
| `~/.claude_token_ring_usage_log.jsonl` | `{ts, util}` history — up to 17 280 entries (~6 months) |
...
All 101 ring icons (0%–100%) are rendered once into `/tmp/claude_ring_icons/r000.png` … `r100.png`. On app startup a background daemon thread fills any missing entries via `_prerender_icons()`. During regular ticks we just set `self.icon = _get_or_render_icon(pct_int)` — pure path-switch, no PIL work, no disk writes.
...
| `RATE_LOG_MAX` | 17,280 | Max usage-log entries (~6 months at 30 s ticks) |
